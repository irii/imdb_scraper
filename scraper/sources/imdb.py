...
class ImdbSource(ScraperSource):
    ...
    def _parse_list(self, container: ScrapeResult, link: str, id: str, soup: BeautifulSoup):
        ...
        lister_list = soup.findAll("div", attrs={'class': 'lister-item mode-detail'})
        ...
        for entry in lister_list:
            ...
            if(match):
                queue.append('https://www.imdb.com/name/' + match.group('Id') + '/bio')     # Queue entry
            ...
            if(match):
                queue.append('https://www.imdb.com/title/' + match.group('Id')) # Queue entry
    ...
    def _parse_actor(self, container: ScrapeResult, link: str, id: str, soup: BeautifulSoup):
        ...
        if(bornInfo):
            ...
            bornInEl = bornInfo.parent.find('a', {'href': _ACTOR_FIND_BIRTH_LOCATION_PARSER}) #bornInfo.find_next_sibling('a').text.strip()
            if(bornInEl):
            ...
            if (el.name == 'table'):
                ...
                for a in rows:
                    ...
                    if(winner_el):
                        ...
                        category = a.find('span', attrs={'class': 'award_category'}).text
                    ...
                    if(movie_link):
                        ...
                        if(movie_link):
                            movie_link_match = _MOVIE_ID_PARSER.match(movie_link['href'])
                            if(movie_link_match):
                                movie_id = movie_link_match.group('Id')
                                queue.append('https://www.imdb.com/title/' + movie_id)
    ...
    def _parse_movie(self, container: ScrapeResult, link: str, id: str, soup: BeautifulSoup):
        ...
        if(posterEl):
            container.images['movie_' + imageUrl] = posterEl.find('img')['src']
